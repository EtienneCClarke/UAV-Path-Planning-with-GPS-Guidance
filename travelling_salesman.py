...
def generateGraph(destinations):
    """
    Generates a dictionary which stores the distance from each node to every other node

    :param destinations: List of coordinate objects
    :return OrderedDict: Ordered dictionary of each node and its adjacent nodes and weights
    """
...
def nearestNeighbour(g):
    """
    Calculates a close to optimal path to visit each node when supplied a weighted graph

    :param g: Ordered Dictionary generated by generateGraph()
    :return Tuple: Returns a tuple of the route and the route_cost. route is a list of destinations- the order is the path
    """
